...
import { AlertCircle, FileEdit, Save, Trash2 } from 'lucide-react';
            ...
            <div className="p-3">
                ...
                {!allSelected ? (
                    ...
                    <div className="space-y-4">
                        <div className="mb-2 flex items-center justify-between rounded-md border bg-muted/50 p-3">
                            <div className="space-y-1">
                                <h2 className="text-base font-medium tracking-tight">
                                    {contexto?.materia.nombre}
                                    <span className="ml-2 font-normal text-muted-foreground">({contexto?.seccion.nombre})</span>
                                </h2>
                                <p className="flex items-center gap-2 text-sm text-muted-foreground">
                                    <Badge variant="outline" className="font-normal">
                                        {contexto?.unidad.nombre}
                                    </Badge>
                                    <span>
                                        Valor total acumulado:{' '}
                                        <span className={sumaActual >= 100 ? 'font-semibold text-destructive' : 'font-semibold text-foreground'}>
                                            {sumaActual}/100 pts
                                        </span>
                                    </span>
                                </p>
                                    ...
                                    <TableBody>
                                        {tareas.map((t) => (
                                            <TableRow key={t.id}>
                                                ...
                                                <TableCell className="text-xs text-muted-foreground">{t.descripcion || '—'}</TableCell>
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4 py-2">
                        ...
                        <div className="space-y-1.5">
                            ...
                            />
                            {!tareaDialog.tarea && (
                                <p className="mt-1 text-xs text-muted-foreground">Puntos disponibles para asignar: {100 - sumaActual}</p>
                            )}
                        </div>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setTareaDialog({ open: false, tarea: null })}>
                            Cancelar
                        </Button>
                        <Button onClick={saveTarea} disabled={isSaving || !nombre || !valor}>
                            Guardar Tarea
                        </Button>
                    </DialogFooter>
                </DialogContent>
            ...
            <Dialog open={calificarDialog.open} onOpenChange={(o) => !o && setCalificarDialog({ open: false, tarea: null })}>
                <DialogContent className="flex max-h-[90vh] max-w-3xl flex-col">
                    <DialogHeader className="shrink-0">
                        ...
                        <DialogDescription>Valor máximo: {calificarDialog.tarea?.valor} pts. Guarde los cambios al terminar.</DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter className="shrink-0 pt-4">
                        <Button variant="outline" onClick={() => setCalificarDialog({ open: false, tarea: null })}>
                            Cerrar
                        </Button>
