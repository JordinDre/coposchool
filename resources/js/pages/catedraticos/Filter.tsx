...
export default function Filter({ secciones, materias }: FilterProps) {
    const { url } = usePage();
    ...
    useEffect(() => {
        ...
        setFilters({
            search: s,
            seccion_id: p.get('seccion_id') || undefined,
            materia_id: p.get('materia_id') || undefined,
            status: p.get('status') || undefined,
        });
        isInitialMount.current = false;
    }, [url]);

    const activeCount = [filters.seccion_id, filters.materia_id, filters.status].filter(Boolean).length + (search ? 1 : 0);
    ...
    const applyFilters = useCallback(() => {
        ...
        router.get(route('catedraticos.index'), params, { preserveState: true, replace: true });
    }, [search, filters]);
    ...
    const clearFilters = () => {
        ...
        setFilters({ search: '' });
        setIsOpen(false);
        router.get(route('catedraticos.index'), {}, { preserveState: true, replace: true });
    };
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <div className="relative flex-1">
                    ...
                    <Input
                        ...
                        onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); if (search.trim()) { applyFilters(); } else { clearFilters(); } } }}
                        className="pl-10"
                    />
                </div>
                <div className="flex gap-2">
                    <Dialog open={isOpen} onOpenChange={(o) => { setIsOpen(o); if (!o) applyFilters(); }}>
                        ...
                        <DialogContent>
                            <DialogHeader><DialogTitle>Filtros Avanzados</DialogTitle></DialogHeader>
                            <div className="grid gap-4 py-4">
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="grid gap-2">
                                        <Label>Sección</Label>
                                        <Select value={filters.seccion_id || ''} onValueChange={(v) => setFilters((p) => ({ ...p, seccion_id: v || undefined }))}>
                                    ...
                                    <div className="grid gap-2">
                                        <Label>Materia</Label>
                                        <Select value={filters.materia_id || ''} onValueChange={(v) => setFilters((p) => ({ ...p, materia_id: v || undefined }))}>
                                            <SelectTrigger><SelectValue placeholder="Todas" /></SelectTrigger>
                                            <SelectContent>
                                                {materias.map((m) => (
